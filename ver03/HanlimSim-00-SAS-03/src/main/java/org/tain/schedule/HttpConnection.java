...
public class HttpConnection {
	...
	public static String getResponse(final Map<String,Object> root) throws Exception {
		...
		try {
			...
			if (Boolean.TRUE) {
				...
				if (flag) System.out.println("----> getDefaultUseCaches(): " + con.getDefaultUseCaches());
				if (flag) System.out.println("----> getConnectTimeout(): " + con.getConnectTimeout());
				if (flag) System.out.println("----> getReadTimeout(): " + con.getReadTimeout());
			}
			...
			if (Boolean.TRUE) {
				...
				if (properties != null) {
					...
					if (flag) System.out.println("----> properties: " + properties);
				}
			}
			...
			if (!flag) {
				...
				if (flag) System.out.println("----> authorization: " + authorization);
			}
			
			String request = null;
			if (Boolean.TRUE) {
				request = (String) root.get("request");
				if (flag) System.out.printf("----> request1(%d): %s\n", request.length(), request);
			...
			if (Boolean.TRUE) {
				...
				dos.flush();
				dos.close();
				if (flag) System.out.printf("----> request2(%d): %s\n", request.length(), request);
			}
			...
			if (flag) {
				...
				if (flag) System.out.println("----> resCode: " + resCode);
			}
		...
		if (flag) System.out.println("----> response: " + sb.toString());
		return sb.toString();
	}
